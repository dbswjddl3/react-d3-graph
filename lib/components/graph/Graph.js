...
var Graph = /*#__PURE__*/ (function(_React$Component) {
  ...
  function Graph(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "_zoomed", function() {
      console.log("zoomed or panning"); // const transform = d3Event.transform;
    ...
    _defineProperty(_assertThisInitialized(_this), "onClickGraph", function(e) {
      ...
      var highlightedNode = _this.state.highlightedNode;

      if (highlightedNode) {
        _this.state.config.nodeHighlightBehavior && _this._setNodeHighlightedValue(highlightedNode, false);
      }
    ...
    _defineProperty(_assertThisInitialized(_this), "onClickNode", function(clickedNodeId) {
      if (_this.state.config.collapsible) {
        ...
        if (highlightedNode === clickedNodeId) {
          _this.state.config.nodeHighlightBehavior && _this._setNodeHighlightedValue(clickedNodeId, false);
        } else {
          if (highlightedNode) {
            _this.state.config.nodeHighlightBehavior && _this._setNodeHighlightedValue(highlightedNode, false);
          }

          _this.state.config.nodeHighlightBehavior && _this._setNodeHighlightedValue(clickedNodeId, true);
        }
      }
    });
